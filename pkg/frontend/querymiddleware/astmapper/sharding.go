...
func NewQueryShardSummer(ctx context.Context, shards int, squasher Squasher, logger log.Logger, stats *MapperStats) (ASTMapper, error) {
	summer, err := NewShardSummer(ctx, shards, squasher, logger, stats, newQueryShardLabeller(shards))
	if err != nil {
		return nil, err
	}
...
func NewShardSummer(ctx context.Context, shards int, squasher Squasher, logger log.Logger, stats *MapperStats, shardLabeller ShardLabeller) (*shardSummer, error) { //nolint:revive // We do not need shardSummer to be exported.
	if squasher == nil {
